          ...
          unless v.empty?
            ...
            when "range"
              if params[:latitude].present? && params[:longitude].present?
                filters["Range"] = v + " miles"
              end
            when "building_availability"
              filters["Building Availability"] = v
            when "classroom_availability"
              filters["Classroom Availability"] = v
            when "latitude"
              if params[:longitude].present?
                result = Geocoder.search([params[:latitude], params[:longitude]]).first
                if result
                  address = result.data["address"]
                  street = [address["house_number"], address["road"]].compact.join(" ")
                  location = [street, address["city"], address["country"], address["postcode"]].compact.join(", ")
